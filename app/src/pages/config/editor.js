...
module.exports = function(params) {
    ...
    function load() {
        if (self.isNewObs()) {
            ...
            return serverService.getAppConfigElement(params.id, params.revision)
                .then(binder.assign('configElement'))
                .then(binder.update())
                .then(titleUpdated)
    ...
    function updateAfterPersist(response) {
        ...
        window.history.pushState({}, document.title, 
            `#/configs/${self.idObs()}/revisions/${self.revisionObs()}/editor`);
    }
    ...
    self.save = function(vm, event) {
        ...
        let identityChanged = (self.configElement.id !== self.idObs()) || 
