...
function formatActivityArray(activityMap, schedule) {
  ...
  switch (schedule.scheduleType) {
    ...
    case "once":
      if (hasTimesOfDay) {
        return formatActivityPhrase(activityMap, `do #{label} at ${timeString}`, ` #{count} times`, `#{list}`);
      }
      return formatActivityPhrase(activityMap, "do #{label}", " #{count} times", `#{list}`);
  }
}
